...
(select 
    ...
    sum(tgs.puntreturns) as "No",
    sum(tgs.puntreturnyards) as yards,
    round(sum(tgs.puntreturnyards) / sum(tgs.puntreturns), 2) as avg,
    max(tgs.puntreturnlong) as "long",
    sum(tgs.puntreturntds) as "td" 

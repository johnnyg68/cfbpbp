-- Defense points per play allowed

SELECT 
   row_number() over (order by t.PointsPerPlay asc) as '#',
    t.name AS 'Team',
    t.teamid as 'TeamId',
...
select
    ...
    ap_poll.ranking,
   	...
   	sum(tgs2.rushyards) + sum(tgs2.passyards)  as 'Yards',
   	sum(tgs2.points) as 'Points',
   	(sum(tgs2.rushyards) + sum(tgs2.passyards)) / (sum(tgs2.rushatts) + sum(tgs2.passatts)) as 'YardsPerPlay',

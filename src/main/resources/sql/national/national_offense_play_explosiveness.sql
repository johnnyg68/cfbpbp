...
select
    ...
    round(count(case when play.playstatyardage >= 20 then 1 end) / count(play.offenseteamid) * 100, 2) as Pct
from
	play
...
where
    ...
    play.playtypeid not in (8, 12, 59, 65, 73, 20, 21, 52, 53)
group by
	team.teamid,
    game.year
) as t

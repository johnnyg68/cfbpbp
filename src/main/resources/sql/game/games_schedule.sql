-- game/game_schedule.sql
-- All completed and future games for week & year

select
	date_format(game.date, "%Y-%m-%d") as SqlDate,
	date_format(game.date, "%m-%d-%Y") as Date,
    awayconf.name as 'VisitorConf',
...
from
    ...
    join conference as homeconf on homeconf.conferenceid = hometeam.conferenceid
    join conference as awayconf on awayconf.conferenceid = awayteam.conferenceid
where
	game.year = :year and
	game.week = :week
union
select
	date_format(schedule.date, "%Y-%m-%d") as SqlDate,
	date_format(schedule.date, "%m-%d-%Y") as Date,
    awayconf.name as 'VisitorConf',
	if(ap_away.ranking is null, 
		awayteam.name, 
		concat("#", ap_away.ranking, " ", awayteam.name)) as Visitor,
	"TBD" as 'Visitor Score',
    homeconf.name as 'HomeConf',
	if(ap_home.ranking is null, 
		hometeam.name, 
		concat("#", ap_home.ranking, " ", hometeam.name)) as Visitor,
	"TBD" as 'Home Score',
...
from
    ...
    join conference as homeconf on homeconf.conferenceid = hometeam.conferenceid
    join conference as awayconf on awayconf.conferenceid = awayteam.conferenceid
    left join ap_poll as ap_away on awayteam.teamid = ap_away.teamid and ap_away.year = :year
    left join ap_poll as ap_home on hometeam.teamid = ap_home.teamid and ap_home.year = :year
where 
	schedule.year = :year and
	schedule.week = :week and 
	schedule.complete = 0
order by
	SqlDate asc;